...
import { Box, Typography, Paper, Table, TableHead, TableRow, TableCell, TableBody, Button, Dialog, DialogTitle, DialogContent, DialogActions, TextField, MenuItem, Select, InputLabel, FormControl, IconButton, Grid, Alert, Chip, CircularProgress } from '@mui/material';
...
const Products = () => {
  ...
  const handleOpen = async (product = null) => {
    ...
    if (product) {
      ...
      try {
        ...
        setCurrentProduct({ ...initialFormState, code, unit: settings?.units[0] || '' });
      } catch (err) {
  ...
  const downloadSampleCSV = () => {
    const sampleData = [ { name: 'Product Name', category: 'Grocery', unit: 'kg', price: '50' } ];
  ...
  const canManage = loggedInUser.role === 'ADMIN' || loggedInUser.role === 'MANAGER';
  ...
  return (
    <Box>
      ...
      <Paper>
        <Table>
          <TableHead><TableRow>
            <TableCell>Code</TableCell><TableCell>Name</TableCell><TableCell>Category</TableCell><TableCell>Unit</TableCell><TableCell>Price</TableCell>
      ...
      {/* --- THIS IS THE RESTORED DIALOG COMPONENT --- */}
      <Dialog open={importDialog.open} onClose={handleImportClose} fullWidth maxWidth="md">
        <DialogTitle>Import Products from CSV</DialogTitle>
        <DialogContent>
          <Box sx={{ mb: 3 }}>
            <Typography variant="body2" sx={{ mb: 2 }}>
              Upload a CSV file with columns: <strong>name, category, unit, price</strong>
            </Typography>
            <Button variant="outlined" onClick={downloadSampleCSV} startIcon={<DownloadIcon />} sx={{ mr: 2 }}>
              Download Sample
            </Button>
          ...
          {importDialog.errors.length > 0 && (
            <Alert severity="error" sx={{ mb: 2 }}>
              ...
              {importDialog.errors.map((error, idx) => (
                <Typography key={idx} variant="body2">Row {error.row}: {error.errors.join(', ')}</Typography>
              ))}
            </Alert>
          )}
          ...
          {importDialog.data.length > 0 && (
            <Box>
              <Typography variant="subtitle1" sx={{ mb: 2 }}>Preview ({importDialog.data.length} products to import):</Typography>
              <Paper sx={{ maxHeight: 300, overflow: 'auto' }}>
                <Table size="small">
                  <TableHead><TableRow><TableCell>Name</TableCell><TableCell>Category</TableCell><TableCell>Price</TableCell></TableRow></TableHead>
                  <TableBody>
                    {importDialog.data.slice(0, 10).map((product, idx) => (
                      <TableRow key={idx}>
                        <TableCell>{product.name}</TableCell>
                        <TableCell>{categories.find(c => c.id === product.productCategoryId)?.name}</TableCell>
        ...
        <DialogActions>
          ...
          <Button onClick={handleImportConfirm} variant="contained" disabled={importDialog.data.length === 0 || importDialog.errors.length > 0}>
            Import {importDialog.data.length} Products
          </Button>
        </DialogActions>
      </Dialog>
      {/* --- END OF RESTORED DIALOG COMPONENT --- */}
    </Box>
  );
};
