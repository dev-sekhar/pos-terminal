...
import { purchaseSchema } from '@pos-terminal/schemas/dist/purchaseSchema';
...
router.post('/', rbacMiddleware(PERMISSIONS.MANAGE_PURCHASES), validate(purchaseSchema), purchasesController.createPurchase);
...
router.put('/:id', rbacMiddleware(PERMISSIONS.MANAGE_PURCHASES), validate(purchaseSchema), purchasesController.updatePurchase);
